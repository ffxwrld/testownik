  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err instanceof z.ZodError) {
        const msg = err.issues[0]?.message || t('common.error', 'Błąd walidacji');
        setError(msg);
        toast.error(msg);
      } else {
        const msg = (err as Error)?.message || t('common.error', 'Error');
        setError(msg);
        toast.error(msg);
      }
    } finally {
      ...
      <div className="max-w-md w-full bg-white dark:bg-zinc-900 shadow-sm rounded-xl shadow-xl p-8 border border-zinc-200 dark:border-zinc-800">
        <div className="flex items-center justify-between mb-2">
          <h2 className="text-2xl font-bold text-zinc-900 dark:text-zinc-50">
            {t('auth.almostDone', 'Prawie gotowe!')}
          </h2>
          <button
            type="button"
            onClick={handleExit}
        ...
        <p className="text-zinc-500 dark:text-zinc-400 mb-6">
          {t('auth.setupSubtitle', 'Zalogowano pomyślnie. Wybierz swój unikalny nick, który będzie widoczny dla znajomych i w rankingach.')}
        </p>

        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label htmlFor="username" className="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1">
              {t('auth.usernameLabel', 'Nick (nazwa użytkownika)')}
            </label>
            <input
              id="username"
              type="text"
              ...
              placeholder={t('auth.usernamePlaceholder', 'np. testownik_ninja')}
              minLength={3}
              maxLength={30}
              pattern="^[a-zA-Z0-9_]+$"
              title={t('auth.usernameAllowedHint', 'Dozwolone są tylko litery (bez polskich znaków), cyfry i znak podkreślenia.')}
            />
          </div>
          ...
          <div className="text-center pt-2">
            ...
            >
              {t('auth.loggedAsOther', 'Zalogowano jako inny użytkownik?')}{' '}
              <span className="underline">{t('auth.signOut', 'Wyloguj się')}</span>
            </button>
          </div>
        </form>
