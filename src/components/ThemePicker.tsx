          ...
          {THEMES.map((t) => (
            <button
              ...
              className={`w-8 h-8 rounded-full flex items-center justify-center transition-transform duration-200 ${
                theme === t.id ? 'scale-110 ring-2 ring-offset-2 ring-offset-white dark:ring-offset-zinc-900 ring-zinc-400 dark:ring-zinc-500' : 'hover:scale-110'
              }`}
            >
              <div className={`w-6 h-6 rounded-full ${t.colorClass} shadow-sm`} />
            </button>
          ))}
          {/* Arrow */}
