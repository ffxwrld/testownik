...
export const QuestionRenderer: React.FC<QuestionRendererProps> = ({
    ...
    const loadImages = async () => {
      ...
      for (const fileName of Array.from(extractedFiles)) {
        ...
        if (localImages) {
          ...
          // Fetch from IndexedDB (try exact, then try to find case-insensitive if possible, but idb-keyval doesn't support regex search)
          // We will rely on exact match or common extensions we appended.
          blob = await getSessionImage(sessionId, fileName);
          
          // Case sensitivity fallback for local extensions
          if (!blob && !hasImgTags) {
  ...
  // Parse text into parts (text and images)
