#!/bin/bash
cat << 'INNER_EOF' > src/components/social/FriendsView.tsx
...
const FriendCard: React.FC<{
  ...
  const hue = friend.profile.username.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0) % 360;
    ...
    >
      <Card className="flex items-center justify-between bg-white dark:bg-zinc-900 shadow-sm border-zinc-200 dark:border-zinc-800">
        ...
        <div className="flex gap-2">
          {isIncoming && (
            <>
              <Button size="sm" variant="primary" onClick={() => onRespond(friend.friendshipId, true)}>
                {t('social.friends.acceptBtn', 'Akceptuj')}
              </Button>
              <Button size="sm" variant="danger" onClick={() => onRespond(friend.friendshipId, false)}>
                {t('social.friends.rejectBtn', 'Odrzuć')}
              </Button>
            </>
          )}
          {isOutgoing && (
            <Button size="sm" variant="danger" onClick={() => { onRemove(friend.friendshipId); toast.success('Anulowano zaproszenie'); }}>
              Anuluj
            </Button>
          )}
          {friend.status === 'accepted' && (
            <Button size="sm" variant="secondary" onClick={() => { onRemove(friend.friendshipId); toast.success('Usunięto znajomego'); }}>
              Usuń
            </Button>
          )}
        </div>
  ...
  return (
    <div className="flex-1 bg-transparent text-zinc-900 dark:text-zinc-50 p-6 flex flex-col items-center">
      <div className="w-full max-w-2xl">
        ...
        {/* Search / Add Friend */}
        <Card className="mb-8 bg-white/80 dark:bg-zinc-900/80 shadow-sm border-zinc-200 dark:border-zinc-800 rounded-xl">
          ...
          <form onSubmit={handleSearch} className="flex gap-2">
            <input
              ...
              className="flex-1 px-4 py-2 bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-xl text-zinc-900 dark:text-zinc-50 focus:outline-none focus:ring-2 focus:ring-primary-500 font-medium"
              minLength={3}
            />
